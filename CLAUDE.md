...
- Speed comes from prefetch (`prefetchAll` + viewport strategy) plus view transitions: a client-side navigation lands in under 30ms. The felt cost is animation, not network, so the root view-transition is capped at 90ms and `scroll-behavior: smooth` is scoped to `html:focus-within` instead of being a global default. Do not reintroduce a global smooth scroll or a long cross-fade.
